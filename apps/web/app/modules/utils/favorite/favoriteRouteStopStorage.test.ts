...
describe('favoriteRouteStopStorage', () => {
  ...
  it('loads normalized favorite route stops from storage and filters malformed items', () => {
    const storage = {
      getItem: vi.fn(() =>
        JSON.stringify([favoriteRouteStop, { favoriteId: 'broken-favorite' }]),
      ),
      removeItem: vi.fn(),
    }
  ...
  it('writes favorite route stops into localStorage', () => {
    ...
    expect(localStorage.getItem(FAVORITE_ROUTE_STOPS_STORAGE_KEY)).toBe(
      JSON.stringify([favoriteRouteStop]),
    )
  })
