...
const favoriteSlice = createSlice({
  ...
  reducers: {
    setFavoriteRouteStops: (
      state,
      action: PayloadAction<FavoriteRouteStop[]>,
    ) => {
    ...
    addFavoriteRouteStop: (state, action: PayloadAction<FavoriteRouteStop>) => {
      ...
      if (
        !state.routeStops.some(
          (routeStop) => routeStop.favoriteId === newRouteStop.favoriteId,
        )
      ) {
    ...
    removeFavoriteRouteStop: (
      ...
      state.routeStops = state.routeStops.filter(
        (routeStop) => routeStop.favoriteId !== routeStopToRemove.favoriteId,
      )
    },
  },
  selectors: {
