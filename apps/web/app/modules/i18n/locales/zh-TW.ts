    ...
    pages: {
      ...
      settings: {
        ...
        analyticsDescription:
          '這個網站會使用 Google Analytics 搜集匿名使用資料，協助了解哪些功能常被使用、哪些路線被搜尋，以改善服務。',
        analyticsDataNotice:
          '我們不會紀錄你的目前位置、姓名或聯絡資訊；搜尋關鍵字與開啟的公車路線可能會被搜集。你可以用下方開關關閉分析功能。',
    ...
    components: {
      ...
      routeMap: {
        ...
        vehicleMarkerAriaLabel:
          '{{plateNumb}}，最近站牌 {{stopName}}，預估到站 {{estimateLabel}}',
        vehiclePopup: {
          recentStop: '最近站牌',
    ...
    messages: {
      ...
      routeRealtime: {
        ...
        noRealtimeData: {
          title: '即時公車',
          description:
            '目前沒有可顯示的即時公車資訊，可能是已收班或上游暫時未提供完整資料。',
        },
      },
      busService: {
        unauthorized: {
      ...
      geo: {
        permission: {
          ...
          denied: {
            title: '無法使用附近站牌',
            description:
              '附近站牌功能需要位置權限，請在瀏覽器設定中允許此網站存取您的位置資訊。',
          },
        },
        error: {
          permissionDenied: {
          ...
          positionUnavailable: {
            title: '定位失敗',
            description:
              '目前無法取得您的位置，因此暫時無法使用附近站牌。請稍後再試，並確認裝置的定位服務與網路連線是否正常。',
          },
          timeout: {
            title: '定位失敗',
            description:
              '取得位置逾時，因此暫時無法使用附近站牌。請稍後再試，並確認裝置的定位服務與網路連線是否正常。',
          },
        },
      },
    },
